...
class DynamicTracker(Node):
    def __init__(self):
        super().__init__('lidar_semantic_tracker')
        ...
        # ----- Classe dalla CAMERA (fa morire i muri) -----
        # Il centroide di ogni cluster LiDAR viene proiettato nell'immagine di
        # segmentazione: se cade su una classe dinamica (persona/bici) e' un
        # pedone, altrimenti e' scartato. Un muro non e' mai "persona" -> non
        # genera traccia, e il centroide che slitta non fa piu' danni.
        self.declare_parameter('seg_topic', '/semantic/segmentation')
        self.declare_parameter('camera_info_topic', '/camera/camera_info')
        self.declare_parameter('dynamic_classes', [11, 12, 17, 18])
        self.declare_parameter('sample_radius_px', 3)     # intorno campionato attorno al centroide proiettato
        self.declare_parameter('min_dyn_frac', 0.25)      # frazione di pixel su classe dinamica per accettare
        self.declare_parameter('keep_unclassifiable', True)  # cluster fuori FOV camera: tenuti (copertura 360)
        ...
        self.dynamic_classes = list(gp('dynamic_classes').value)
        self.sample_radius_px = int(gp('sample_radius_px').value)
        self.min_dyn_frac = float(gp('min_dyn_frac').value)
        self.keep_unclassifiable = bool(gp('keep_unclassifiable').value)
        ...
        self.create_subscription(CameraInfo, gp('camera_info_topic').value, self.caminfo_cb, qos)
        ...
        self.get_logger().info(
            f'lidar_semantic_tracker avviato (LiDAR posizione+velocita, camera classe) | '
    ...
    def seg_cb(self, msg):
        try:
            self.seg_img = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough')
    ...
    def classify_cluster(self, wx, wy):
        """Classifica un cluster dal suo centroide, proiettandolo nell'immagine
        di segmentazione. Ritorna:
          True  -> cade su classe dinamica (persona/bici): e' un pedone;
          False -> cade su classe non dinamica (muro, ecc.): da scartare;
          None  -> non classificabile (fuori FOV, manca seg/intrinseci/TF):
                   decide keep_unclassifiable.
        La posizione e la velocita' restano quelle del LiDAR: la camera dice
        solo SE tenere il cluster, non DOVE si trova. Cosi' si prende il meglio
        dei due -- misura pulita dal LiDAR, classe dalla camera."""
        if self.seg_img is None or self.K is None or self.cam_frame is None:
            return None
        # mondo -> frame ottico camera
        T = self.lookup_to(self.cam_frame, self.world_frame)
        if T is None:
            return None
        Minv = np.linalg.inv(transform_to_matrix(T))
        pc = Minv @ np.array([wx, wy, 0.0, 1.0])   # nel frame ottico (z avanti)
        Z = pc[2]
        if Z <= 0.05:
        ...
        ui = int(round(u * sx)); vi = int(round(v * sy))
        if ui < 0 or ui >= W or vi < 0 or vi >= H:
            return None                            # fuori inquadratura
        # campiono un intorno e conto i pixel di classe dinamica
        r = self.sample_radius_px
        u0, u1 = max(0, ui - r), min(W, ui + r + 1)
        v0, v1 = max(0, vi - r), min(H, vi + r + 1)
        patch = self.seg_img[v0:v1, u0:u1]
        if patch.size == 0:
            return None
        dyn = np.isin(patch, self.dynamic_classes)
        frac = float(dyn.mean())
        return frac >= self.min_dyn_frac

    def lookup_to(self, target, source, stamp=None):
        try:
    ...
    def cluster_points(self, pts):
        ...
        for idxs in groups.values():
            ...
            # CLASSE DALLA CAMERA: scarta i cluster che cadono su classe non
            # dinamica (muri). Quelli fuori FOV o non classificabili si tengono
            # o no secondo keep_unclassifiable (copertura 360 gradi).
            cls = self.classify_cluster(float(centroid[0]), float(centroid[1]))
            if cls is False:
                continue                       # muro: il centroide puo' slittare quanto vuole, non lo tracciamo
            if cls is None and not self.keep_unclassifiable:
                continue
